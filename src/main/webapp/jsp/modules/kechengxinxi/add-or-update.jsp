...
<body>
						...
						<div class="widget-area-2 lochana-box-shadow">
							...
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>上课时间</label>
										<div id="shangkeshijianDate" class="input-append date form_datetime">
											<input id="shangkeshijian-input" name="shangkeshijian" size="15" type="text" readonly>
									...
									<div class="form-group col-md-6">
										<label>上课地点</label>
										<input name="shangkedidian" id="shangkedidian" name="shangkedidian" class="form-control" placeholder="上课地点">
									</div>
									<div class="form-group col-md-6">
										<label>人数</label>
										<input name="renshu" id="renshu" name="renshu" class="form-control" placeholder="人数">
									</div>
									<div class="form-group col-md-6">
										<label>工号</label>
									...
									<div class="form-group col-md-6">
										...
										<input name="jiaoshixingming" id="jiaoshixingming" name="jiaoshixingming" class="form-control" placeholder="教师姓名">
											...
											<div class="form-group  col-md-12" >
												<label>课程内容</label>
												<div><div class="upload"><input id="kechengneirongupload" name="file" type="file">选择文件</div>
												<script id="kechengneirongEditor" type="text/plain"></script>
	...
	<script>
		...
		function upload(){
			...
			$('#kechengneirongupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('kechengneirongEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
		...
		function setContent(){
			if(ruleForm.kechengneirong != null && ruleForm.kechengneirong != 'null' && ruleForm.kechengneirong != ''){
				var kechengneirongMes = '' + ruleForm.kechengneirong;
				var kechengneirongUeditor = UE.getEditor('kechengneirongEditor');
				kechengneirongUeditor.ready(function() {
					kechengneirongUeditor.setContent(kechengneirongMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('kechengneirongEditor').hasContents()){
				$('#kechengneirong-input').attr('value',UE.getEditor('kechengneirongEditor').getContent());
			}
		}
		...
		function dateTimePick(){
			...
			$("#shangkeshijianDate").datetimepicker({ 
				minView: "month",
				autoclose: true,
				format:'yyyy-mm-dd',
				language:'zh-CN',
		...
		function dataBind(){
			...
			$("#shangkeshijian-input").val(ruleForm.shangkeshijian);
			$("#shangkedidian").val(ruleForm.shangkedidian);	
			$("#renshu").val(ruleForm.renshu);	
